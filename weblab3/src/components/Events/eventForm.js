...
const EventForm = ({ 
  ...
  return(
    ...
    <div className="event__form">
      ...
      <div className="event">{
      events.map( holi =>
        event(holi.id, holi.date.toString(), holi.type, holi.notes, onEventDeleted)  
        )
      }
      </div>
    </div>
...
const event = (key, date, type, comments, onEventDeleted) =>(
  <div key={key} 
  className="individual__event">
    <button className="delete__event__btn" onClick ={ 
      () => 
      onEventDeleted(key)}> 
      {'X'}</button>
    ...
    <label className= "type">{"Event Type: " + type}</label>
     <br />
